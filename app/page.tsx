...
import { 
  ...
  Bot, ChevronRight, Zap, Code2, Layers, CheckCircle2, Rocket, Globe
} from 'lucide-react';
    ...
    <main className="min-h-screen bg-[#0B0C10] text-slate-300 font-sans selection:bg-orange-500/30 selection:text-orange-200 overflow-x-hidden relative">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden flex items-center justify-center">
        ...
        <motion.div
            ...
            animate={{ opacity: 0.05, scale: [1, 1.1, 1], rotate: 360 }}
            transition={{ opacity: { duration: 1 }, scale: { duration: 10, repeat: Infinity, repeatType: "reverse" }, rotate: { duration: 60, repeat: Infinity, ease: "linear" }}}
      ...
      <section className="relative pt-48 pb-32 px-4 z-10">
        <div className="max-w-6xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-orange-500/30 bg-orange-500/10 text-orange-400 text-[10px] font-mono mb-8">
            <span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-orange-400 opacity-75"></span><span className="relative inline-flex rounded-full h-2 w-2 bg-orange-500"></span></span> SYSTEM ONLINE V2.0
          </motion.div>
          ...
          <p className="text-lg md:text-xl text-slate-400 mb-12 max-w-2xl mx-auto leading-relaxed font-light">
            Lupakan Excel manual. Kami mengubah data lapangan menjadi kode, dan kode menjadi efisiensi profit. <span className="text-white font-medium">SaaS. AI. Automation.</span>
          </p>

          <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
            <Link href="/calculator" className="group relative px-8 py-4 bg-orange-600 text-white font-bold rounded-xl overflow-hidden">
            ...
            <Link href="#products" className="px-8 py-4 border border-white/10 rounded-xl text-white font-bold hover:bg-white/5 transition-colors flex items-center gap-2">VIEW BLUEPRINTS</Link>
          </div>

          {/* 3D DASHBOARD PREVIEW (BAGIAN YG ANDA TANYAKAN) */}
          <motion.div initial={{ opacity: 0, y: 50, rotateX: -20 }} animate={{ opacity: 1, y: 0, rotateX: 0 }} transition={{ duration: 1, delay: 0.5 }} className="mt-16 mx-auto max-w-5xl perspective-1000">
            <div className="relative rounded-xl border border-white/10 bg-[#15161A] shadow-2xl shadow-orange-500/10 overflow-hidden transform rotate-x-12 hover:rotate-x-0 transition-transform duration-700 ease-out p-2">
                ...
                {/* --- TEMPEL LINK GAMBAR DASHBOARD DISINI --- */}
                <div className="aspect-video bg-slate-900 relative border-t border-white/5">
                    <img 
                        src="https://wsubscnhhabhtubdorhd.supabase.co/storage/v1/object/public/logos/dashboard%20estimator.png" 
                        alt="App Screenshot" 
                        className="w-full h-full object-cover opacity-90"
                    />
                    {/* Overlay Gradient halus */}
                    <div className="absolute inset-0 bg-gradient-to-t from-[#15161A] via-transparent to-transparent pointer-events-none"></div>
                </div>
            </div>
      ...
      <section id="services" className="py-32 px-4 relative z-10">
        <div className="max-w-7xl mx-auto">
            <div className="mb-16"><h2 className="text-3xl md:text-5xl font-bold text-white mb-4">ENGINEERED FOR <span className="text-slate-600">IMPACT.</span></h2></div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 bg-[#15161A] border border-white/5 p-8 rounded-3xl hover:border-orange-500/30 transition-all group relative overflow-hidden">
                    ...
                    <LayoutDashboard className="w-10 h-10 text-white mb-6" /><h3 className="text-2xl font-bold text-white mb-2">Custom SaaS Development</h3>
                    <p className="text-slate-400 max-w-md">Kami membangun Sistem Operasi lengkap untuk perusahaan Anda. Dari Logistik, HRIS, hingga Keuangan Proyek.</p>
                </div>
                <div className="bg-[#15161A] border border-white/5 p-8 rounded-3xl hover:border-blue-500/30 transition-all group">
                    <Smartphone className="w-10 h-10 text-blue-500 mb-6" /><h3 className="text-xl font-bold text-white mb-2">Mobile Field Ops</h3>
                    <p className="text-slate-400 text-sm">Aplikasi untuk Personil di Lapangan. Input data Laporan Realtime (Offline Mode).</p>
                </div>
                <div className="md:col-span-3 bg-[#15161A] border border-white/5 p-8 rounded-3xl hover:border-purple-500/30 transition-all flex flex-col md:flex-row items-center gap-8">
                    <div className="flex-1">
                        <div className="flex items-center gap-3 mb-4"><Bot className="w-8 h-8 text-purple-500" /><h3 className="text-xl font-bold text-white">AI Automation</h3></div>
                    ...
                    <div className="flex-1 w-full bg-black/30 rounded-xl p-4 border border-white/5 font-mono text-xs text-green-400">
                        &gt; Processing invoice... OK<br/>&gt; Updating inventory... OK<br/>&gt; Sending report to Owner... SENT.
                    </div>
                </div>
            </div>
        </div>
      ...
      <section id="products" className="py-32 px-4 border-t border-white/5 relative z-10">
        <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {tools.map((tool) => (
                    <div key={tool.id} className="group bg-[#15161A] border border-white/5 rounded-2xl overflow-hidden hover:translate-y-[-5px] transition-all">
                        ...
                        <div className="p-6">
                            ...
                            {tool.demo_url ? <Link href={tool.demo_url} className="block w-full py-3 bg-white text-black text-center font-bold text-sm rounded-lg hover:bg-slate-200 transition-colors">LIVE DEMO</Link> : <button disabled className="block w-full py-3 bg-slate-800 text-slate-500 text-center font-bold text-sm rounded-lg cursor-not-allowed">DEV MODE</button>}
                        </div>
                    </div>
                ))}
            </div>
      ...
      {/* 5. VISION SECTION (BARU: Menjawab pertanyaan tombol Vision) */}
      <section id="vision" className="py-32 px-4 bg-gradient-to-b from-[#0B0C10] to-black relative z-10 border-t border-white/5">
        <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-3xl md:text-5xl font-bold text-white mb-8">OUR <span className="text-orange-600">VISION</span></h2>
            <p className="text-xl text-slate-400 mb-16 max-w-2xl mx-auto">
                Transisi Perusahaan Manual Barbasis Ai. Kami membawa presisi digital untuk efisiensi total.
            </p>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
                <div className="p-6 border-l-2 border-orange-600 bg-white/5 rounded-r-xl">
                    <div className="mb-4 text-orange-500"><Rocket className="w-8 h-8"/></div>
                    <h4 className="font-bold text-white mb-2">Phase 1: Digitize</h4>
                    <p className="text-sm text-slate-400">Mengubah semua form kertas dan laporan manual menjadi aplikasi cloud yang bisa diakses dari mana saja.</p>
                </div>
